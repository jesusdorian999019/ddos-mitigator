...
class Detector:
    def __init__(self):
        ...
        self.cache_ips: Dict[str, float] = {}  # Anti-spam TTL 60s
    ...
    def escanear_y_detectar(self) -> List[Dict]:
        ...
        for ip, pps, proto in top:
            ...
            if es_anomalia:
                ...
                # Enriquecer anti-spam (TTL 60s)
                if enriquecedor and (ip not in self.cache_ips or now - self.cache_ips[ip] > 60):
                    self.cache_ips[ip] = now
                    tipo = razon.split(' (')[0]
